...
def get_tickers(fiat="ALL"):
    ...
    try:
        ...
        if isinstance(contents, list):
            markets = [(x['market'], x['korean_name']) for x in contents]

            if fiat == "KRW":
                return [(x, y) for x, y in markets if x.startswith("KRW")]
            elif fiat == "BTC":
                return [(x, y) for x, y in markets if x.startswith("BTC")]
            elif fiat == "ETH":
                return [(x, y) for x, y in markets if x.startswith("ETH")]
            elif fiat == "USDT":
                return [(x, y) for x, y in markets if x.startswith("USDT")]
            else:
                return markets
        else:
            return None
